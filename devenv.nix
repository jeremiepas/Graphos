    ...
    "openspec:apply" = {
      exec = ''
        ...
        while [ "$ITER" -lt "$MAX_ITER" ]; do
          ...
          # Run opencode with cloud model (fast, no local prefill bottleneck)
          # Local gemma4 was too slow for opencode's 4k+ token system prompt
          if [ "$ITER" -eq 1 ]; then
            opencode run \
              --model "''${OPENSPEC_MODEL:-ollama/glm-5.2:cloud}" \
              --auto \
