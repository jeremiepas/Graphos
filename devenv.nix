...
in
{
  ...
  tasks = {
    ...
    # Requires both processes to be up on montech:
    ...
    "openspec:apply" = {
      exec = ''
        ...
        while [ "$ITER" -lt "$MAX_ITER" ]; do
          ...
          # Use `script` to fake a TTY — opencode exits immediately without one
          if [ "$ITER" -eq 1 ]; then
            script -qec "opencode run \
              --model \"executor/qwen3.8-executor\" \
              --auto \
              \"You are an autonomous agent. Apply the OpenSpec change '$CHANGE' using the openspec-apply-change skill. Steps: 1) Run openspec instructions apply --change \"$CHANGE\" --json 2) Read all context files 3) For each undone task: implement it using tools (edit, bash, read, grep), then mark it [x] in tasks.md 4) Do NOT ask for clarification — make decisions autonomously 5) Do NOT stop until all tasks are done or you hit a fatal error. Execute now.\"" /dev/null >> "$LOGFILE" 2>&1
          else
            script -qec "opencode run \
              --model \"executor/qwen3.8-executor\" \
              --auto \
              --continue \
              \"Continue autonomously. Pick up the next undone task from '$CHANGE' and implement it using tools. Mark it [x] in tasks.md. Do NOT ask for clarification. Do NOT stop until all tasks are done or you hit a fatal error.\"" /dev/null >> "$LOGFILE" 2>&1
          fi
          EXIT_CODE=$?
