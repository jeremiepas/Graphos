...
writeCommunityNote g dir cohesion (cid, members) = do
  let filename = "_COMMUNITY_" ++ show cid ++ ".md"
      ...
      memberList = T.intercalate ", " [formatNodeLink nb | nid <- members
                                          , let nb = Map.findWithDefault (Node nid "unknown" CodeFile "" Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing) nid (gNodes g)]
      content = T.unlines
