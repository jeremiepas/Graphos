...
handleQueryGraph g idx args = do
  let question = textArg args "question"
      ...
      mScope = textArgMaybe args "path"
  if T.null question
    then pure (Left "Missing required argument: question")
    else do
      let ctl = defaultBudgetCtl { bcByteBudget = budget }
          resp = queryGraphWithIndexScoredScoped g idx (toCachedFGL g) question mode budget mScope
...
allTools =
  [ ("query_graph", "Query the knowledge graph using BFS or DFS traversal. Returns verdict, best_score, hash, ranked nodes/edges, and omitted counts. Set edges=semantic (default) to drop AMBIGUOUS/trivia edges; edges=all preserves everything. Set max_nodes to cap returned nodes; set budget (bytes) to cap serialized node size; set max_label_chars to truncate labels. Set path to a source-file glob (e.g. 'src/cli/**') to restrict candidate matches to that scope before traversal, so the budget is spent only on in-scope nodes.", [("question", "The search question", True), ("mode", "bfs or dfs", False), ("budget", "Byte budget for serialized nodes (default: 2000)", False), ("edges", "semantic or all", False), ("max_nodes", "Cap on returned nodes (0 = unlimited)", False), ("max_label_chars", "Max label length before truncation (0 = default 120)", False), ("path", "Optional source-file glob restricting candidate matches (e.g. 'src/cli/**')", False)])
