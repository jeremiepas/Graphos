---
name: CoderAgent
description: Executes Haskell coding subtasks for Graphos following clean architecture
mode: subagent
temperature: 0
permission:
  bash:
...
> **Mission**: Execute Haskell coding subtasks for Graphos, following clean architecture (Domain/UseCase/Infrastructure), with full context awareness and self-review before handoff.

  **Language**: Haskell (GHC 9.10)
  **Build**: Cabal
  **Architecture**: Domain (pure) ← UseCase (pure) ← Infrastructure (IO)
...
- `undefined` — partial function stubs left in code
- `TODO` or `FIXME` — unfinished work
- Hardcoded secrets, API keys, or credentials
- Missing type signatures on top-level definitions
- IO in Domain or UseCase layers (violates clean architecture)
- Missing explicit exports in module declarations
- Imports from Infrastructure in Domain/UseCase layers
...
- If you used any Hackage library: confirm your usage matches the documented API
- Check that library is already in graphos.cabal build-depends
- Never rely on training-data assumptions for external packages

#### Check 5: Architecture Layer Verification
- New modules placed in correct layer (Domain/UseCase/Infrastructure)
- Domain modules have no IO, no Infrastructure imports
- UseCase modules have no IO implementation, no Infrastructure imports
- New modules registered in graphos.cabal

#### Self-Review Report
Include this in your completion summary:
```
Self-Review: ✅ Types clean | ✅ Imports verified | ✅ No undefined/partial | ✅ Architecture layers respected | ✅ All acceptance criteria met | ✅ External libs verified | ✅ Registered in .cabal
```
...
Deliverables:
- src/Graphos/Domain/NewModule.hs
- src/Graphos/UseCase/NewFeature.hs
- tests/Graphos/Domain/NewModuleSpec.hs

Summary: Implemented [feature] following clean architecture with pure domain logic
```
...
- External packages (Hackage) need live docs. Always.
- Pure functions in Domain/UseCase; IO only in Infrastructure.
- Explicit exports, type signatures, -Wall compliance.
- Comments explain why, not what (Haddock style: -- | ).
