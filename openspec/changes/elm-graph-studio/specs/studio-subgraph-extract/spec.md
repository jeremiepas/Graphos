# studio-subgraph-extract

Carve a piece of the graph out and keep it: select a scope, preview it, export it as a
normalized view JSON in the `json-graph-web-view` canonical shape — downloadable, and
directly usable by the existing views catalog (drop into `graphos-out/views/`).
