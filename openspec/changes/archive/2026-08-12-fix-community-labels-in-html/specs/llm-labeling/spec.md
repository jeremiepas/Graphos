...
Extend LLM community labeling so it is available on `graphos ingest` and so the resulting labels flow to both `graph.json` and `graph.html` (previously only the full pipeline produced labels, and only `graph.json` carried them — the HTML viewer ignored them entirely).
