...
### Requirement: Embeddings persisted to graph output

The pipeline SHALL persist per-node embedding vectors to an `embeddings.json` sidecar in the
output directory and SHALL record the sidecar path in `graph.json` via an optional
`embeddings_path` field. The graph loader SHALL follow the pointer and load embeddings when
the sidecar exists, returning `gEmbeddings = Nothing` when the pointer is absent or the file
is missing. Legacy `graph.json` files without the pointer SHALL load unchanged.

#### Scenario: Graph with embeddings sidecar
- **WHEN** `graph.json` contains `"embeddings_path": "embeddings.json"` and `embeddings.json`
  exists with valid vectors
- **THEN** `loadGraphFromFile` returns a `LoadResult` whose graph has `gEmbeddings = Just ...`
  containing the vectors

#### Scenario: Legacy graph without pointer
- **WHEN** `graph.json` has no `embeddings_path` field
- **THEN** `loadGraphFromFile` returns a `LoadResult` whose graph has `gEmbeddings = Nothing`
  and the load succeeds without error
...
(`--embed` was used or `embedding.enabled == true`). The `--no-semantic-edges` CLI flag SHALL
override the config to `false` for one run.

#### Scenario: Embeddings disabled skips semantic pass
- **WHEN** `embedding.enabled == false` and `--embed` is not passed
