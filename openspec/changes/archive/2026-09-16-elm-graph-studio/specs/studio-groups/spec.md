...
Query+color groups in the studio, with the semantics specified by `query-groups`
(first-match-wins precedence, isolate/hide, persistence, import/export) — evaluated
server-side when connected, locally in file mode.
