...
- **Plan**: Reduce peak runtime memory to <8GB on 100k-node, multi-language codebases (measured via `+RTS -s`). Current peak: 40-60GB+ on such workloads.
- **Do**: Implement the seven fixes listed above, each independently testable.
- **Check**: Run `graphos .` on a 50k+ file multi-language codebase with `+RTS -s` and verify peak memory <8GB. Run existing Hspec test suite. Benchmark extraction throughput before/after sequential LSP change.
- **Act**: Standardize memory-aware patterns (bounded buffers, incremental merge, compact types) as project conventions in code-quality.md. Feed learnings into future pipeline optimization work.