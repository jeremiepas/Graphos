...
## Purpose
Defines how `graphos query` scores and ranks results: normalized matched-term
scores, the query-term full-label boost, zero scores for non-matching neighborhood
context, and score-descending ordering with matched nodes above context neighbors.
Refines the query workflow in `openspec/specs/04-query`.
## Requirements
