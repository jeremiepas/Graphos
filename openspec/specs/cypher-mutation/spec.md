...
The openCypher write clause subset evaluated against the in-memory property
graph: `CREATE`, `MERGE` (with `ON CREATE SET` / `ON MATCH SET`), `SET`,
`REMOVE`, `DELETE` / `DETACH DELETE`, optionally combined with `MATCH` /
`WHERE` and an optional trailing `RETURN`. Mutations apply in memory by
default and are persisted to `graph.json` only on explicit opt-in; every
surface is read-only unless the write context is explicitly selected.

## Purpose

Let agents and users curate the knowledge graph with the same standard
query language they use to query it — locally, without a graph database,
with deterministic model-reconciliation semantics and a mutation summary.
