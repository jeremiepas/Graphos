...
- **WHEN** a query contains `CREATE`, `MERGE`, `SET`, or `DELETE`
- **THEN** the query is rejected with an error naming the unsupported clause
- **AND** the loaded graph is left unchanged

#### Scenario: out-of-subset construct reports position
- **WHEN** a query uses a construct outside the supported grammar
