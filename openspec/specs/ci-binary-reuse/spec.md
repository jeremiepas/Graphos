...
Speed up CI and eliminate redundant binary builds: cache the cabal store between runs, publish the built graphos binary as a workflow artifact, and let the repo analyzer reuse release assets or CI artifacts instead of building from source every dispatch.
