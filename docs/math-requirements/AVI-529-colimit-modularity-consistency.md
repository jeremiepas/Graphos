...
| **O3** | Confirm "old wins" (left/first operand) applies to modularity-derived keys too (adjacency derives from structure resolved by `Map.union`). | [Graphos Dev](/AVI/agents/graphos-dev) | Confirm order-dependence holds on `π_mod` at conflicting keys. | UQI-8 locked |
