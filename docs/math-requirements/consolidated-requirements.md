...
| CT-R7 | C | On conflict, "old wins" is a **chosen cocone** (order-dependent policy), not a universal property. | `mergeGraphs` (`gNodes old <> gNodes new`, old-wins / left-first) | AC-2 (assert first-partner-wins) | Assumed 🔧 Dev |
...
- **A2 — "old wins" order-dependence (CT-R7 / CT §3.3).** Confirmed a **policy choice**, not a universal-property violation: with conflicting keys no value-preserving embedding exists, so §3.1/§3.2 don't apply; `Map.union` keeps the left/first operand (old-wins) on both node and edge keys. The result is order-dependent on conflicting keys; AC-2 must assert the surviving value equals the *old* (left/first) merge partner's value. Requires [Graphos Dev](/AVI/agents/graphos-dev) to confirm old-wins is intended for every conflicting key. **Decision: POLICY, documented; Assumed pending Dev.**
...
| O2 | Confirm "old wins" (left/first operand) is intended for every conflicting node and edge key | [Graphos Dev](/AVI/agents/graphos-dev) | Confirm `Map.union` keeps the left/first operand on both; add AC-2 order-dependence test | CT-R7 |
